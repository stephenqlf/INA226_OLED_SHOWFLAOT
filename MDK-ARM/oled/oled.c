...
// 幂函数辅助 (计算 10 的 n 次方)
static unsigned long oled_pow(unsigned char m, unsigned char n) {
 ...
 * @brief 显示浮点数
 * @param x: 起始列 (0-127)
 * @param y: 起始行 (0-7, 对应像素行 0, 16, 32...)
 * @param num: 浮点数值
 * @param len: 整数部分的长度 (例如 12.34 长度为 2, 123.45 长度为 3)
...
void OLED_ShowFloat(u8 x, u8 y, float num, u8 len, u8 size) {
    ...
    // 1. 处理负数 (如果 OLED 字体支持负号，这里可以扩展，暂时假设都是正数或简单处理)
    if (num < 0) {
        OLED_ShowChar(x, y, '-', size, 1);
        x += (size/2); // 移动光标
        num = -num;
        // 如果是负数，整数部分长度逻辑可能需要微调，这里简化处理
    }

    // 2. 分离整数和小数部分
    // 技巧：num * 100 + 0.5 实现四舍五入
    long temp_val = (long)(num * 100.0f + 0.5f);
    
    integer_part = temp_val / 100;      // 整数部分
    fractional_part = temp_val % 100;   // 小数部分 (0-99)

    // 3. 格式化字符串
    // %d.%02d 自动处理小数点后的补零 (如 5 变成 05)
    sprintf(temp, "%d.%02d", integer_part, fractional_part);

    // 4. 显示字符串
    OLED_ShowString(x, y, (unsigned char*)temp, size, 1);
}
